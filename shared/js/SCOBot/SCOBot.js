        ...
        if (!isStarted) {
            ...
            if (settings.mode === "review" || settings.entry === '' || settings.entry === 'resume') { // Resume, or possible Resume
                ...
                if (settings.suspend_data.length > 0 && !isBadValue(settings.suspend_data)) {
                    ...
                    try {
                        setting.suspend_data = JSON.parse(settings.suspend_data); // Turn this back into a object.
                        scorm.debug(settings.suspend_data, 4);
                    }
                    catch (e) {
                        scorm.debug(settings.prefix + ": Unable to parse suspend_data to a JSON object; returning suspend_data as is", 4);
                    }
    ...
    this.setInteraction = function (data) {
        if (isStarted) {
            ...
            if ($.type(data.timestamp) === "date") {
                ...
                var ta = data.timestamp.toISOString();
                timestamp = ta.substr(0, 21) + 'Z';
            }
...
// Add definition for toISOString if there is none defined
